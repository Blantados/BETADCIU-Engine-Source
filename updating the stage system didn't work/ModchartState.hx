...
class ModchartState 
{
    ...
    public function new(path:String, ?isStageLua:Bool = false)
    {
		...
		Lua_helper.add_callback(lua,"setScrollFactor", function(obj:String , scrollX:Float, scrollY:Float) {
			if(getObjectDirectly(obj) != null) {
				trace('check');
				var shit:FlxObject = getObjectDirectly(obj);
		...
		Lua_helper.add_callback(lua, "close", function(thing:Bool) {
			closed = true;
			return closed;
		});
		...
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0, updateHitbox:Bool = true) {
			if(getObjectDirectly(obj)!=null) {
				var shit:FlxSprite = getObjectDirectly(obj);
		...
		Lua_helper.add_callback(lua, "doTweenX", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = getObjectDirectly(vars);
			cancelTween(tag);
			if(penisExam != null) {
	...
	public function getThing(id:String) // works for scrollFactor?
	{ 
		var shit:Dynamic;
		...
		else if(PlayState.instance.modchartInputTexts.exists(id))
			shit = PlayState.instance.modchartInputTexts.get(id);
	...
	{	
		...
		if (PlayState.instance.Stage.isCustomStage)
		{
			for (script in PlayState.instance.luaArray)
			{
				if (script.scriptName == PlayState.instance.Stage.stageScriptName)
				{
					PlayState.instance.luaArray.remove(script);
					script.die();
				}
			}
		}
