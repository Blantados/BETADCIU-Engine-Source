...
class StageModchartState 
{
		...
		Lua_helper.add_callback(lua, "getProperty", function(variable:String) {
			...
			if(killMe.length > 1) 
			{
				...
				if(PlayState.instance.modchartSprites.exists(killMe[0]))
					...
					coverMeInPiss = PlayState.instance.Stage.swagBacks.get(killMe[0]);
				else if(PlayState.instance.modchartInputTexts.exists(killMe[0]))
					coverMeInPiss = PlayState.instance.modchartInputTexts.get(killMe[0]);
	...
	function tweenShit(tag:String, vars:String) {
		...
		if(PlayState.instance.modchartIcons.exists(variables[0]))
			sexyProp = PlayState.instance.modchartIcons.get(variables[0]);
		if(PlayState.instance.modchartCharacters.exists(variables[0]))
			sexyProp = PlayState.instance.modchartCharacters.get(variables[0]);
