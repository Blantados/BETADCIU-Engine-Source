    ...
    public function new(path:String, ?preloading:Bool = false, ?isStageLua:Bool = false)
    {
		...
		else
		{
			...
			Lua_helper.add_callback(lua, "removeLuaSprite", function(tag:String, destroy:Bool = true) {
				...
				var pee:ModchartSprite = (Stage.instance.swagBacks.exists(tag) ? Stage.instance.swagBacks.get(tag) : PlayState.instance.modchartSprites.get(tag));
				if(destroy) {
					pee.kill();
				}
	...
	public function callOnCompleted(type:String = "tween", tag:String, ?loops:Int, ?loopsLeft:Int)
	{
		switch (type.toLowerCase())
		{
			case 'timer':
				...
				if (PlayState.instance.Stage.isCustomStage && PlayState.instance.Stage.luaArray.length >= 1)
					PlayState.instance.Stage.callOnLuas("onTimerCompleted", [tag, loops, loopsLeft]);
			default:
				PlayState.instance.callOnLuas("onTweenCompleted", [tag]);
