package backend.system.sound.waveform;
...
class WaveformDataParser
{
  ...
  public static function parseWaveformDataString(contents:String, ?fileName:String):Null<WaveformData>
  {
    ...
    try
    {
        var parsed:WaveformData = tjson.TJSON.parse(contents);
        return parsed;
    }
    catch (e:Dynamic)
    {
  ...
  static function printError(error:Dynamic, id:String = ''):Void
  {
    trace('[WAVEFORM] Failed to parse waveform data: ${id}');
    trace('[WAVEFORM] Error details: ${error}');
  }
}