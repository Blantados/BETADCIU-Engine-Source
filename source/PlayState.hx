...
class PlayState extends MusicBeatState
{
			...
			{	
				...
				if (!daNote.modifiedByLua)
				{
					if (FlxG.save.data.downscroll || daNote.downscroll)
					{
						if (daNote.mustPress)
							{
							center = playerStrums.members[Math.floor(Math.abs(daNote.noteData))].y + Note.swagWidth / 2;
						...
						{
							center = cpuStrums.members[Math.floor(Math.abs(daNote.noteData))].y + Note.swagWidth / 2;
							daNote.y = (cpuStrums.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * roundedSpeed);
						}
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		if (SONG.song.toLowerCase() == 'bonedoggle' && mania == 3) //force only four pressable keys
		{
			holdArray = [controls.LEFT, controls.DOWN, controls.UP, controls.RIGHT];
			pressArray = [controls.LEFT_P, controls.DOWN_P, controls.UP_P, controls.RIGHT_P];
			releaseArray = [controls.LEFT_R, controls.DOWN_R, controls.UP_R, controls.RIGHT_R];	
