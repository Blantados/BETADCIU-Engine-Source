...
class FlxAnimateFrames extends FlxAtlasFrames
{
    ...
    public static function fromTextureAtlas(Path:String):FlxAtlasFrames
    {
        ...
        else
        {
            ...
            if (Assets.exists('$Path/Animation.json')){
                texts = Assets.list(TEXT).filter((text) -> StringTools.startsWith(text, '$Path/spritemap'));
            }else{
                texts = FileSystem.readDirectory('$Path/').filter((text) -> StringTools.startsWith(text, '$Path/spritemap'));
                isAssets = false;
            }
            ...
            for (text in texts)
            {
                ...
                spritemaps.push({image: (isAssets ? Assets.getBitmapData('$Path/${json.meta.image}') : BitmapData.fromFile('$Path/${json.meta.image}')), json: json});
            }
    ...
    public static function fromSpriteMap(Path:FlxSpriteMap, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        ...
        if (Path is String)
        {
            ...
            var isAssets = Assets.exists(str);
            json = haxe.Json.parse((StringTools.contains(str, "/")) ? (isAssets ? Assets.getText(str) : File.getContent(str)) : str);
        }
        else
            json = Path;
    ...
    public static function fromEaselJS(Path:String, ?Image:FlxGraphicAsset):FlxAtlasFrames
    {
        ...
        var isAssets = Assets.exists(Path);
        var separatedJS = (isAssets ? Assets.getText(Path) : File.getContent(Path)).split("\n");
        ...
        for (i in 0...names.length)
        {
            ...
            var name = names[i];
            var json = jsons[i];
            var bitmap = FlxG.bitmap.add(isAssets ? Assets.getBitmapData((Image == null) ? '${imagePath.join("/")}/${json.images[0]}' : Image) : BitmapData.fromFile((Image == null) ? '${imagePath.join("/")}/${json.images[0]}' : Image));
